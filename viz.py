...
def fetch(package, max_depth, current_depth=0):
    ...
    if requires:
        req_packages = [strip(req) for req in requires]
        dependencies[package] = req_packages
        for dep in req_packages:
            if dep:
                sub_deps, new_depth = fetch(dep, max_depth, current_depth + 1)
...
def visualize(program_path, package, output_path, max_depth):
    deps, _ = fetch(package, max_depth)
    graph = build(deps)
...
def main():
    ...
    visualize(args.program_path, args.package, args.output_path, args.max_depth)
